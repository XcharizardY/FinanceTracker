"""
Finance Tracker Pro
A modern desktop application built with PyQt6, pandas, and matplotlib.
"""
...
class AiFeatures(Protocol):
    def analyze_spending(self, df: pd.DataFrame) -> str: ...
    def suggest_category(self, description: str, categories: list[str]) -> str | None: ...
    def budget_advice(self, df: pd.DataFrame, budgets: dict) -> str: ...
    def chat_assistant(self, df: pd.DataFrame, question: str) -> str: ...
    def detect_unusual_spending(self, df: pd.DataFrame) -> str: ...
...
AUTOSAVE_PATH = os.path.join(PROJECT_ROOT, "data", "autosave.csv")
LOGO_PATH = os.path.join(PROJECT_ROOT, "assets", "FinanceTracker.png")
...
QPushButton:hover {{ background: {BRAND_YELLOW_HOVER}; }}
...
class FinanceApp(QWidget):
    ...
    def load_autosave_if_present(self):
        if not os.path.exists(AUTOSAVE_PATH): return
        try:
    ...
    def init_ui(self):
        ...
        content_layout.addLayout(self.create_dashboard())
    ...
    def create_sidebar(self):
        ...
        logo = QLabel()
        if os.path.exists(LOGO_PATH):
            logo.setPixmap(QPixmap(LOGO_PATH).scaled(30, 30, Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation))
    ...
    def create_dashboard(self):
        ...
        self.income_card = self.create_card("Total Income", "#00c8ff", text_color="#white")
        self.expense_card = self.create_card("Total Expenses", "#ff4d6d", text_color="white")
        self.savings_card = self.create_card("Net Savings", "#7cff6b", text_color="white")
        # Fixed Contrast: Uses Brand Yellow text instead of invisible black text
        self.rate_card = self.create_card("Savings Rate", BRAND_YELLOW, text_color=BRAND_YELLOW)
    ...
    def create_card(self, title, accent_color, text_color="white"):
        frame = QFrame()
        ...
        label_value.setStyleSheet(f"font-size: 22px; font-weight: bold; color: {text_color}; border: none;")
    ...
    def update_chart(self):
        """Renders cash flow line chart and clean donut chart with side percentages."""
        plt.close("all")
        fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(6, 5.5), gridspec_kw={'height_ratios': [1.2, 1]})
        ...
        ax1.set_title("Cash Flow", color="white", fontsize=11, pad=10)
        ax1.set_xlabel("") 
        ...
        # Donut Chart with clean side legend (no raw slice overlaps)
        category = df[df["Type"] == "Expense"].groupby("Category")["Amount"].sum()
        if not category.empty:
            total_val = category.sum()
            wedges, _ = ax2.pie(
                category.values, 
                colors=PIE_COLORS, 
                wedgeprops={"edgecolor": "#161616", "linewidth": 2.0, "width": 0.45}, 
                startangle=90
            )
            
            # Formatted legend labels containing category and percentage
            legend_labels = [f"{cat} ({val/total_val*100:.0f}%)" for cat, val in zip(category.index, category.values)]
            ax2.legend(
                wedges, 
                legend_labels, 
                loc="center left", 
        ...
        ax2.set_title("Expenses by Category", color="white", fontsize=11, pad=10)

        fig.tight_layout(pad=1.5, h_pad=3.0)
        self.canvas.figure = fig
        self.canvas.draw()
    ...
    def _check_ai_ready(self):
        if ai_features is None:
            QMessageBox.warning(self, "AI Error", f"ai_features.py import failed.\nDetails: {AI_IMPORT_ERROR}")
            return False
        return True

    def _run_ai_task(self, button, worker, on_success):
        button.setEnabled(False)
        self.ai_thread = AIWorker(worker)
        
        def handle_success(result):
            on_success(result)
            
        def handle_error(err_msg):
            self._show_ai_error(err_msg)

        self.ai_thread.finished_signal.connect(handle_success)
        self.ai_thread.error_signal.connect(handle_error)
    ...
    def _show_ai_insights(self, result):
        ...
        QMessageBox.information(self, "AI Financial Insights", result)
    ...
    def _show_ai_budget(self, result):
        ...
        QMessageBox.information(self, "AI Budget Advisor", result)
    ...
    def _show_ai_fraud(self, result):
        ...
        QMessageBox.information(self, "AI Fraud Detection", result)

    def run_ai_chat(self):
        if not self._check_ai_ready(): return
        q, ok = QInputDialog.getMultiLineText(self, "AI Assistant Chat", "Ask a question about your finances:")
        if ok and q.strip():
    ...
    def _show_ai_chat(self, result):
        ...
        QMessageBox.information(self, "AI Assistant Chat", result)
    ...
    def _show_ai_error(self, message):
        for btn in [self.ai_insights_btn, self.ai_category_btn, self.ai_budget_btn, self.ai_fraud_btn, self.ai_chat_btn]:
            btn.setEnabled(True)
        QMessageBox.warning(self, "AI Execution Note", message)
